...
if __name__ == '__main__':
    ...
    for i in range(len(vmlist)):
        if i < (len(vmlist) / 2):
            ...
            subprocess.Popen("ssh ubuntu@{} '~/cpu/testcases/5/iambusy 250000'".format(iplist[i]), stdout=FNULL, shell=True)
        else:
            print('{} start running 50%.'.format(vmlist[i]))
            subprocess.Popen("ssh ubuntu@{} '~/cpu/testcases/5/iambusy 30000'".format(iplist[i]), stdout=FNULL, shell=True)
